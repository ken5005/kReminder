...
`docs/kReminder_Config仕様_v1.md` は `config.properties` の正典。**存在する全キーの一覧を
持つことが仕様の一部**になっている（`Config.save()` は Config が知っているキーだけを
書き出すため、一覧に無いキーは手で書いても次の保存で消える）。

**このPRで `Config` にキーを追加・削除・改名したら、Config仕様 §3 のキー一覧表も必ず
更新すること。** 値の読み書きロジックだけの変更なら触らなくてよい。
