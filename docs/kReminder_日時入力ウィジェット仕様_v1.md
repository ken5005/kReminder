# kReminder 実行時刻の日時入力ウィジェット仕様書 v1.3（③-e）

最終更新: 2026-07-28（N11/N10 対応で v1.3 に更新）
GUI仕様 v2 §4.8 から分冊（本書が正典・v2 §4.8 はポインタ）。chat31 で仕様確定・chat32 で実装 ship 済み（feat/datetime-field・PR#13）。v1.1 で §3.2/§7 の「新規時デフォルト」を確定（新規/複製/削除スライス）。v1.2 で §3.4/§3.5/§6/§7 の Enter・Space の抜け方を改訂（feat/datetime-field-exit）。v1.3 で §3.5/§3.6/§6 を touched（↑↓／ホイール編集済み）フラグの導入と OK 活性 gate の簡素化に合わせて改訂（feat/wheel-edit-mark）。
...
- **Space**（v1.3 で分岐条件を改訂）＝カーソル欄のバッファ状態・touched 状態で分岐（分岐の中身自体は変更なし）：
  - **バッファ活性中（打ちかけがある）、または現欄を↑↓／ホイールで編集済み（touched）** → その値をゼロ埋め確定して現欄の値は活かし、**下位の欄だけ**を最小値にして確定・カーソル消滅。
  - **どちらでもない**（欄に居るがまだ何も編集していない＝再進入直後や自動送り直後）→ **現欄を含めて**最小値にして確定・カーソル消滅。
  - 最小値＝月/日は 1・時分秒は 0。
...
- **増減した欄は編集済み（touched）扱いとなり、Space の分岐でバッファ活性と同等に扱われる**（→§3.5）。touched はカーソルが欄を離れる／消滅すると解除される。
...
- **OK 活性 = `isTotallyValid` のみ（カーソル活性は問わない）**（v1.3 改訂・N10）。
  改訂理由：`getExecTimeText` はカーソル活性中もゼロ埋め解釈で常に完全な日時を合成するため、gate は不要だった。マウスで OK を押すとフォーカス移動の `focusLost` で閉店処理が先に走るため、Enter で閉店してから OK を押したのと同一経路になる。
- **v1.3 で gate（カーソル無活性条件）を外したため、ダイアログを開いた瞬間から OK は活性**（初期値＝既存値または新規時の現在日時はいずれも valid なため）。v1.2 までの「開いた瞬間は OK 非活性で始まる」はカーソル無活性 gate があった当時の仕様の帰結で、現在は当てはまらない。
  （歴史的経緯）v1.2 で Enter が1回で登録完了するようになったため、「日時をいじらない時も Enter を一手入れる必要がある」という v1.1 までの割り切りは解消された（Enter 1回で閉店＋OK発火が同時に走る）。
